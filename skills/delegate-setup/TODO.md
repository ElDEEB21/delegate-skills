...
| codex | none found | `codex models` is treated as a prompt ("stdin is not a terminal") | no native listing. Option: ship a documented hints list in references/ clearly marked as not discovered, or leave `unsupported` and let users pass IDs manually. |
